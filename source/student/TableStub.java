...
public class TableStub {
	...
	/**
	 * Waiter start to sitting the students on the table
	 * @param id of the thread that is running
	 * @return StudentOrder to let the rest of the program to know about the arrived order
	 */
	public StudentOrder sittingStudent(int id) {
		ClientCom con = new ClientCom (serverHostName, serverPortNumb);
		Message inMessage, outMessage;
	    String inString, outString;
		...
		outMessage = new Message (Message.sSITTING, id);     
		outString = outMessage.toXMLString ();                         // converte resposta para XML
	    con.writeObject (outString);                                   // envia resposta ao servidor
	    inString = (String) con.readObject ();                         // ler resposta do servidor em XML
	    inMessage = new Message (inString);
		int msg = inMessage.getType();
		con.close ();
		if(msg == Message.FIRST) {
			return StudentOrder.FIRST;
		} else if (msg == Message.OTHER) {
			return StudentOrder.OTHER;
		} else {
			return StudentOrder.LAST;
		}
	}
